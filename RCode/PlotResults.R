...
## Subset the data for the distribution you want
MasterRes <- MasterRes[MasterRes$sim=="NegBin",];distr_label <- "NegBin"
#MasterRes <- MasterRes[MasterRes$sim=="BetaBin",];distr_label <- "BetaBin"
    ...
    for (m in unique(MasterRes$m)) {
      ...
      for (sims in "No Compensation") {
        ...
        p <- p + geom_point(data = point,
          ...
          #      title = paste0("M = ", m)) +
          labs(
            y = ifelse(as.numeric(effect) == 3,
                       paste0("M = ", m),
                       ""),
            x = "",
            fill = fillvar,
            title = ifelse(as.numeric(m) == 50,
...
## Prepare Plots
pl <- c(plotlist2[hmp_3], plotlist2[hmp_5])
pl <- c(pl[c(grep("n50", names(pl)),
             grep("100", names(pl)),
             grep("150", names(pl)))])
gg <- ggarrange(
  plotlist = pl,
  ncol = 2,
  nrow = length(unique(MasterRes$m)),
  vjust = .75,
  common.legend = TRUE,
  legend = "bottom"
)
...
ggsave(
  MeanPlot,
  file = paste0("MainSimulationResults_",distr_label,".pdf"),
  width = 75,
  height = 79,
  limitsize = FALSE,
  device = "pdf"
...
df1 <- read.csv(paste0("No Compensation50HMP Mid Vagina3",distr_label,".csv"))
df2 <- read.csv(paste0("No Compensation50HMP Mid Vagina5",distr_label,".csv"))
df3 <- read.csv(paste0("No Compensation100HMP Mid Vagina3",distr_label,".csv"))
df4 <- read.csv(paste0("No Compensation100HMP Mid Vagina5",distr_label,".csv"))
df5 <- read.csv(paste0("No Compensation150HMP Mid Vagina3",distr_label,".csv"))
df6 <- read.csv(paste0("No Compensation150HMP Mid Vagina5",distr_label,".csv"))
...
#df2 <- df2[,1:5]
write.csv(df2, "PlotTables2_",distr_label,".csv", row.names = F)
