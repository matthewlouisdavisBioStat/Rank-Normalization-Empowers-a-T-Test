...
load("C:/Users/Matthew/Documents/Courses/Kai/Final Results/FourSimData/MasterResBetaBin.RData")
    ...
    for (m in unique(MasterRes$m)) {
      ...
      for (sims in "No Compensation") {
        ...
        p <- p + geom_point(data = point,
          ...
          #      title = paste0("M = ", m)) +
          labs(
            y = ifelse(as.numeric(effect) == min(MasterRes$effect),
                       paste0("M = ", m),
                       ""),
            x = "",
            fill = fillvar,
            title = ifelse(as.numeric(m) == min(MasterRes$m),
...
## Prepare Plots
pl <- c(plotlist2[hmp_3], plotlist2[hmp_5], plotlist2[hmp_1.5])
...
df1 <- clean(df1, min, low)
df2 <- clean(df2, min, high)
df3 <- clean(df3, med, low)
df4 <- clean(df4, med, high)
df5 <- clean(df5, max, low)
df6 <- clean(df6, max, high)
...
#df2 <- df2[,1:5]
write.csv(df2, file = paste0("PlotTables_",distr_label,".csv"), row.names = F)